"""
get_dataset_versions.py
 
Retrieve all available versions of a dataset from the EES API.
 
Datasets on EES are versioned — each time a dataset is updated with
new data, a new version is released. This module returns all versions
for a given dataset ID, allowing users to see the version history and
select a specific version to query.
"""
import requests
...
def warning_max_pages(response: Dict[str, Any]):
    ...
    if "paging" in response:
        ...
        current_page = response["paging"].get("page", 1)
         # Warn if requested page is beyond available pages
...
def get_dataset_versions(
    ...
    Returns version history for a given dataset, including version number,
    release date and status. If page is None, automatically paginates
    through all pages and returns combined results.
 
    Parameters
    ----------
    dataset_id : str
        Unique ID of the dataset to get versions for. Required.
    ...
    -------
    list of dict
        List of version dicts, each containing version number,
        type, status and published date.
   ...
   # Build the URL for the first (or only) page request
    url = api_url(
   ...
   # Print the request URL in verbose mode
    if verbose:
        print(f"GET {url}")
    # Send GET request to the dataset versions endpoint
